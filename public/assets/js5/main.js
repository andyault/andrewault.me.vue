...
var util = {
	...
	computedHeight: function computedHeight(elem) {
		...
		parseFloat(styles['border-top-width']) + parseFloat(styles['border-bottom-width']) + parseFloat(styles['margin-top']) + parseFloat(styles['margin-bottom']);
	},
	...
	easeOut: function easeOut(t, b, c, d) {
		if (t == d) return b + c;else return c * 1.001 * (-Math.pow(2, -10 * t / d) + 1) + b;
	},
	...
	setPath: function setPath(path) {
		//fuck you opera mini
		if (window.history.replaceState) window.history.replaceState({}, '', path);else if (path == '/') window.location.hash = '';else window.location.hash = '#' + path;
		...
		if (parts[1]) {
			...
			if (parts[2]) {
				...
				//if we're on a file
				if (parts[3]) title += sep + app.curProject.files[app.gallery.curFile - 1];
			}
		}
	...
	getPath: function getPath() {
		if (window.history.replaceState) return window.location.pathname;else return window.location.hash;
	},
	getContext: function getContext(canvas) {
		...
		if (!context) {
			...
			if (type == 'webgl' && !context) context = canvas.getContext('experimental-webgl');
...
var background = {
	...
	draw: function draw() {
		if (util.getScroll() < background.canvas.offsetHeight) {
			var context = util.getContext(background.canvas, background.BG.context);

			if (context) background.BG.draw(context);
		}

		//only draw if we're on the same background and we're not paused
		if (background.curBackground == background.oldBackground) if (!background.paused) requestAnimationFrame(background.draw);
	},
			...
			if (background.BG.dependencies && background.BG.dependencies.length) {
				var _loop = function _loop(i) {
					...
					dependencies[i] = new Promise(function (resolve, reject) {
						...
						if (dependencies[i - 1]) dependencies[i - 1].then(function () {
							req.send();
						});else req.send();
					});
				};
...
var smoothScroll = {
	...
	think: function think() {
		...
		if (curTime - smoothScroll.startTime < smoothScroll.duration) {
			window.scroll(0, util.easeOut(curTime - smoothScroll.startTime, smoothScroll.startY, smoothScroll.deltaY, smoothScroll.duration));
	...
	to: function to(underlineElem, targetX, preventOverwrite) {
		...
		underlineElem.think = function () {
			...
			if (curTime - startTime < underline.duration) {
				underlineElem.style.left = util.easeOut(curTime - startTime, startX, deltaX, underline.duration) + 'px';

				//make sure to hasn't been called since we started this one
				if (underlineElem.startTime == startTime) requestAnimationFrame(underlineElem.think);
			} else {
...
var app = new Vue({
	...
	methods: {
		...
		viewInfo: function viewInfo(project, elem, force) {
			...
			var siblings = parent.querySelectorAll('a[id]'); //barf but if we use children we'll get #info too
			...
			if (info.oldFirstInRow && firstInRow != info.oldFirstInRow) {
				...
				var oldIndex = parseInt(info.oldFirstInRow.attributes['data-index'].value);
				...
				if (info.oldFirstInRow.parentNode == elem.parentNode && rowNum > oldRowNum) info.style['margin-top'] = oldMargin - 15 + 'px';
			...
			if (info.offsetHeight == 0) arrow.style.left = arrowLeft + 'px';else if (parseFloat(arrow.style.left) != arrowLeft) //can't use offsetLeft because of negative margin
		...
		prevFile: function prevFile(dontChange) {
			if (--this.gallery.curFile <= 0) this.gallery.curFile = this.gallery.numFiles;

			this.viewFile(this.gallery.curFile);
		},
		nextFile: function nextFile() {
			if (++this.gallery.curFile > this.gallery.numFiles) this.gallery.curFile = 1;
		...
		submitForm: function submitForm() {
			...
			//see if we made it
			req.onload = function (e) {
				if (req.status == 200) app.contact.response = 'Thanks! :)';else {
...
//the big stuff
{
	...
	hooks.onReady = function () {
							...
							for (var _iterator = links[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
								...
								if (scroll >= elem.offsetTop && scroll < elem.offsetTop + elem.offsetHeight) {
									if (!isActive) {
										...
										//move underline
										if (!navUnderline.isMoving) underline.to(navUnderline, link.offsetLeft);
									}
		...
		//navigation link hijacking and underline movement
		{
			...
			var _arr = [logo].concat(_toConsumableArray(links));
		...
		Vue.nextTick(function () {
			...
			if (parts[1]) {
				...
				if (parts[2]) {
					...
					var projectElem = document.getElementById(project.cat + '-' + project.project);
					...
					if (parts[3]) Vue.nextTick(function () {
	...
	hooks.onData = function (res) {
		...
		try {
			for (var _iterator3 = Object.keys(res)[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
				...
				try {
					for (var _iterator4 = Object.keys(projects)[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
	...
	hooks.onLoad = function () {
		...
		hooks.onResize = function () {
			...
			if ((parseFloat(info.style.height) || 0) != 0) app.viewInfo(app.curProject, document.getElementById(app.curProject.cat + '-' + app.curProject.project), true);

			if (app.gallery.shouldShow) app.viewFile(app.gallery.curFile);
