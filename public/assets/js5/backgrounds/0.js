...
BG.init = function (canvas) {
	...
	var cw = Math.ceil(canvas.width / life.cellSize);
	var ch = Math.ceil(canvas.height / life.cellSize);

	var ctx = canvas.getContext('2d');

	//translate to center
	ctx.translate(-(cw - canvas.width / life.cellSize) / 2, -(ch - canvas.height / life.cellSize) / 2);

	//more settings

	//start off random cells
	for (var i = 1; i < cw * ch / 5; i++) {
...
BG.draw = function (ctx) {
	...
	var cw = Math.ceil(ctx.canvas.width / life.cellSize);
	...
	if (Date.now() > lastThink + life.thinkInt) {
		...
		for (var i = 0; i < cw * ch; i++) {
			var cell = life.cells[i];

			var neighbors = [life.cells[i - cw - 1], life.cells[i - cw], life.cells[i - cw + 1], life.cells[i - 1], life.cells[i + 1], life.cells[i + cw - 1], life.cells[i + cw], life.cells[i + cw + 1]];
			...
			if (cell) {
				if (count < 2 || count > 3) newCells[i] = undefined;else newCells[i] = true;
			} else if (count == 3) newCells[i] = true;
		}
		...
		if (life.drawGrid) {
			...
			for (var _i = 1; _i < cw; _i++) {
				ctx.fillRect(_i * 8, 0, 1, h);
			}for (var _j = 1; _j < ch; _j++) {
				ctx.fillRect(0, _j * 8, w, 1);
			}
		}

		//draw cells
		for (var _i2 = 0; _i2 < cw * ch; _i2++) {
			if (life.cells[_i2]) {
				var cx = _i2 % cw;
				var cy = Math.floor(_i2 / cw);
