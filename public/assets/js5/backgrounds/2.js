...
BG.think = function (w, h) {
	...
	if (nextCell > w * h) nextCell = nextCell % (w * h);

	if (nextCell < 0) nextCell = nextCell + w * h;
...
BG.draw = function (ctx) {
	...
	} else ant.wasResized = false;

	//think
	for (var _i = 0; _i < ant.thinkCalls; _i++) {
		this.think(w, h);
	...
	for (var _i2 = 0; _i2 < w * h; _i2++) {
		var cell = ant.cells[_i2];

		if (cell != oldCells[_i2]) {
			var cx = _i2 % w;
			var cy = Math.floor(_i2 / w);
