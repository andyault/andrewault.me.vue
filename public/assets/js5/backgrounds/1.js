...
BG.init = function (canvas) {
	...
	var cw = Math.ceil(canvas.width / ant.cellSize);
	var ch = Math.ceil(canvas.height / ant.cellSize);

	var ctx = canvas.getContext('2d');

	//translate to center
	ctx.translate(-(cw - canvas.width / ant.cellSize) / 2, -(ch - canvas.height / ant.cellSize) / 2);

	//random states
	for (var i = 2; i < Math.floor(Math.random() * 8); i++) {
...
BG.think = function (cw, ch) {
	var curState = ant.cells[ant.curPos] || 0;
	...
	if (nextCell > cw * ch) nextCell = nextCell % (cw * ch);

	if (nextCell < 0) nextCell = nextCell + cw * ch;
	...
	if (ant.direction > 3) ant.direction = 0;else if (ant.direction < 0) ant.direction = 3;
...
BG.draw = function (ctx) {
	...
	var cw = Math.ceil(ctx.canvas.width / ant.cellSize);
	var ch = Math.ceil(ctx.canvas.height / ant.cellSize);

	var w = cw * ant.cellSize;
	var h = ch * ant.cellSize;
	...
	if (Date.now() > lastThink + ant.thinkInt) {
		...
		//draw cells
		for (var _i = 0; _i < cw * ch; _i++) {
			var cell = ant.cells[_i];

			if (cell) {
				var cx = _i % cw;
				var cy = Math.floor(_i / cw);

				var hue = cell * (360 / ant.states.length);
		...
		//draw ant
		if (ant.drawAnt) {
			var antx = ant.curPos % cw;
			var anty = Math.floor(ant.curPos / cw);
